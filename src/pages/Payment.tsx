    ...
    const options = {
      ...
      handler: async function (response: RazorpayResponse) {
        try {
          ...
          let alreadyBooked: string[] = [];
          if (movieShowSnap.exists()) {
            alreadyBooked = movieShowSnap.data().bookedSeats || [];
          }

          
          const conflict = bookingData.selectedSeats.some((seat: string) =>
            alreadyBooked.includes(seat)
          );

          if (conflict) {
            alert(
              "Some selected seats have been booked by another user. Please try booking again."
            );
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium dark:text-white text-black">{bookingData.theater}</p>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-300">Show Time:</span>
                    <p className="font-medium dark:text-white text-black">{bookingData.showTime}</p>
                  </div>
                </div>
