  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        (mobileLocationRef.current && mobileLocationRef.current.contains(target))
      ) {
        return;
      }
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="hidden min-[1219px]:flex flex-col w-full py-2">
          <div className="flex flex-row flex-wrap items-center gap-2 w-full">
            ...
            <div className="flex-1 min-w-0 mx-2 max-w-sm xl:max-w-md 2xl:max-w-lg relative" ref={searchRef}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500 truncate">
                                {result.matchType === "description"
                                  ? result.matchText
                                  : `${result.matchType === "cast"
                                    ? "Starring"
                                    : result.matchType === "director"
                                      ? "Directed by"
                                      : "Genre"
                                  }: ${result.matchText}`}
                              </p>
            ...
            <div className="relative flex-shrink-0 w-36 xl:w-44 ml-2 z-50" ref={locationRef}>
              <button
                      ...
                      <div>
                        ...
                        {popularLocations.map((location) => (
                          <button
                            ...
                            className={`w-full px-3 py-3 text-left transition-colors border-b border-gray-100 last:border-b-0 ${String(selectedLocation.id) === String(location.id)
                              ? "bg-red-200"
            ...
            <div className="flex flex-wrap items-center gap-2 ml-2">
              <Link to="/concerts" className="px-3 py-2 bg-gray-800 rounded-md text-white hover:bg-gray-700 transition-colors">Concerts</Link>
              <Link to="/sports" className="px-3 py-2 bg-gray-800 rounded-md text-white hover:bg-gray-700 transition-colors">Sports</Link>
              <Link to="/theatresandarts" className="px-3 py-2 bg-gray-800 rounded-md text-white hover:bg-gray-700 transition-colors">Theatre & Arts</Link>
              <Link to="/bookings" className="px-3 py-2 bg-gray-800 rounded-md text-white hover:bg-gray-700 transition-colors flex items-center">
                <Ticket className="h-5 w-5 mr-1" />
                <span>My Bookings</span>
              </Link>
              <Link to="/offers" className="px-3 py-2 bg-purple-700 rounded-md text-white hover:bg-purple-800 transition-colors">Offers</Link>
            </div>
            ...
            <div className="flex items-center ml-2">
              ...
              >
                {theme === "light" ? (
                  <span className="flex items-center"><span role="img" aria-label="moon">🌙</span> Dark</span>
                ) : (
                  <span className="flex items-center"><span role="img" aria-label="sun">☀️</span> Light</span>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-x-4 mt-2 w-full">
            {currentUser ? (
              ...
              <>
                <Link to="/login" className="px-2 md:px-3 xl:px-4 py-1.5 xl:py-2 text-sm md:text-base xl:text-lg bg-red-600 hover:bg-purple-600 text-white font-medium rounded-md transition-colors">
                  Login
                </Link>
                <Link to="/register" className="px-2 md:px-3 xl:px-4 py-1.5 xl:py-2 text-sm md:text-base xl:text-lg bg-purple-600 hover:bg-red-600 text-white font-medium rounded-md transition-colors">
                  Register
                </Link>
              </>
            )}
        ...
        <div className="flex max-[1218px]:flex min-[1219px]:hidden items-center w-full justify-between py-2">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 focus:outline-none ml-auto">
        ...
        {menuOpen && (
          <div className="max-[1218px]:flex min-[1219px]:hidden absolute top-full left-0 right-0 bg-gray-900 rounded-b-lg shadow-lg z-50 flex flex-col gap-y-2 p-4 border-t border-gray-700">
                        ...
                        <div>
                          ...
                          {popularLocations.map((location) => (
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div>
                                  <div className={`font-medium ${String(selectedLocation.id) === String(location.id)
                                    ? "text-red-700 font-semibold" 
                                    : "text-gray-900" 
                                    }`}>{location.name}</div>
                                  <div className={`text-sm ${String(selectedLocation.id) === String(location.id)
                                    ? "text-red-600"
                                    : "text-gray-500"
                                    }`}>{location.state}</div>
                                </div>
            ...
            <Link to="/bookings" className="w-full text-left px-4 py-2 rounded-md hover:bg-gray-800 transition-colors" onClick={() => setMenuOpen(false)}>My Bookings</Link>
            <Link to="/concerts" className="w-full text-left px-4 py-2 rounded-md hover:bg-gray-800 transition-colors" onClick={() => setMenuOpen(false)}>Concerts</Link>
            <Link to="/sports" className="w-full text-left px-4 py-2 rounded-md hover:bg-gray-800 transition-colors" onClick={() => setMenuOpen(false)}>Sports</Link>
            <Link to="/theatresandarts" className="w-full text-left px-4 py-2 rounded-md hover:bg-gray-800 transition-colors" onClick={() => setMenuOpen(false)}>Theatre & Arts</Link>
            <Link to="/offers" className="w-full text-left px-4 py-2 rounded-md bg-purple-700 text-white hover:bg-purple-800 transition-colors" onClick={() => setMenuOpen(false)}>Offers</Link>
            {!currentUser && (
              <>
                <Link to="/login" onClick={() => setMenuOpen(false)} className="w-full text-left px-4 py-2 rounded-md bg-red-600 hover:bg-purple-600 text-white transition-colors">Login</Link>
                <Link to="/register" onClick={() => setMenuOpen(false)} className="w-full text-left px-4 py-2 rounded-md bg-purple-600 hover:bg-red-600 text-white transition-colors">Register</Link>
              </>
            )}
            <button
              onClick={toggleTheme}
